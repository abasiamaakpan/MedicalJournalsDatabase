...
### Creating the authors dataframe
```{r}
...
### Creating the authors dataframe
```{r}
#author_list_df(author,)
#author_list_df_df <- tibble::rowid_to_column(author_list_df, "authorID")
author_list_df_final=author_list_df %>% distinct(FirstName,LastName,Initial)
#author_list_df_final
```

```{r}
daAuthor <- merge( author_list_df,  author_list_df_final, all.x=TRUE)
daAuthor 
```
```{r}
daAuthor[4]
...
### articleauthors df
```{r}
dArticle_Authors <- tibble::rowid_to_column(daAuthor_new, "authorID")
dArticle_Authors
```
```{r}
daAuthor_final<- subset( daAuthor, select = -c(index,name))
```

### authors df
```{r}
daAuthor_new_frame <- tibble::rowid_to_column(daAuthor_final, "authorID")
daAuthor_new_frame
```
...
```{r}
 elocation_df_new_final <- subset( elocation_df_new, select = -c(index))

```

```{r}
 elocation_df_new_final
```
```{r}
elocation_df_new_final= elocation_df_new_final%>% 
  dplyr::rename(
    articleID = name,
...
```{r}
article_elocation <- subset(elocation_df_new_final, select = -c(articleID))
```

```{r}
article_elocation
```
## Creating the elocationDF
```{r}
 elocation_df_new_final_finished<- subset(elocation_df_new_final, select = -c(type,content))
```

```{r}
 elocation_df_new_final_finished
```
...
```{r}
#require(data.table)
#newdf1 <- data.table(newdf, key="type")
#pubtype_new_df_final <- data.table(pubtype_new_df_final, key="type")

#rslt = pubtype_new_df_final[newdf1,allow.cartesian=TRUE][,list(type = unique(type), articleID=length(articleID)),by=type]
#rslt
...
```{r}
#grant_df_resolved<- #data.frame(articleid=grant_df_in_p_1[2],grantid=grant_df_in_p_1[4],acronym=gra#nt_df_in_p_1[5],agency=grant_df_in_p_1[6],country=grant_df_in_p_1[7])
```


```{r}
#article_pub_type_df <- subset(newdf, select = -c(type))
#article_pub_type_df
```
...
## Grant DF
```{r}
grant_df_resolved_done=grant_df_resolved_final %>% distinct(articleid,grantid,agency,acronym,country)
grant_df_resolved_done
```


```{r}
grant_df_modified <- tibble::rowid_to_column(grant_df_resolved_done, "uniquegrantID")

```


```{r}
grant_df_modified_new <- data.frame(uniquegrandID=grant_df_modified[1],articleid=grant_df_resolved_done[1],grantid=grant_df_resolved_done[2],acronym=grant_df_resolved_done[3],agency=grant_df_resolved_done[4],country=grant_df_resolved_done[5])
```

```{r}
grant_df_modified_new
```
```{r}
#grant_df_modified_new
grant_df_modified[1]
grant_df_modified_new[2]
```


```{r}
grant_final_dataframe <- data.frame(uniquegrantID=grant_df_modified[1],articleid=grant_df_modified_new[2])
```


## Article grant dataframe
```{r}
grant_final_dataframe 
```
...
```{r}
article_df_final <- data.frame(articleid=grabbing_index[1],journalid=article_journal_initial_df[1],pubModel=articlepubModel,articleTitle=articletitle,pagination=articlep,date=articled,language=articlel)
article_df_final
```
