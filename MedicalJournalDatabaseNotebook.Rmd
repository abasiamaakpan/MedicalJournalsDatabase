...
grants_df_temp <- grants_df_temp %>% dplyr::rename(ArticleID = name)

# format columns on grants data frame
grants_df_formatted <- grants_df_temp %>% distinct(ArticleID, GrantID, Agency, Acronym, Country)
...
grants_df <- data.frame(UniqueGrandID = grants_df_new_col[1], ID = grants_df_formatted[2], Acronym = grants_df_formatted[3], Agency = grants_df_formatted[4], Country = grants_df_formatted[5])
...
ArticleIndex <- tibble::rowid_to_column(journals_df_temp, "ArticleID")

article_df <- data.frame(ArticleID = ArticleIndex[1], JournalID = ArticleIndex[1], PubModel = ArticlePubModel, Title = ArticleTitle, Pagination = ArticlePagination, Date = ArticleDate, Language = ArticleLang)

# article_df$Date <- ymd(article_df$Date)
...
```{r}
grabbing_index <- tibble::rowid_to_column(journals_df_formatted, "articleID")
#grabbing_index
#articlel
...
# load articles table
res <- dbSendQuery(con, "INSERT INTO articles (article_id, journal_id, pub_model, title, pub_date, lang) VALUES (:ArticleID, :JournalID, :PubModel, :Agency, :Country);", articles_df)
